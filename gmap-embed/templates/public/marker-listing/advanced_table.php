...
<div class="wpgmap-marker-listing wpgmap-adv-table-listing" style="max-width: <?php echo esc_attr($wpgmap_map_width); ?>; margin: 20px auto 0;">
    ...
    jQuery(document).ready(function ($) {
        ...
        function initDataTable(markers) {
            ...
            var dataSet = markersCache_<?php echo esc_attr($count); ?>.map(function (marker, idx) {
                ...
                var title = escHtml(marker.marker_name || '');
                if (marker.distance) {
                    title += ' <div class="wgm-dist-badge-wrap"><span class="wgm-marker-distance-badge">' + escHtml(marker.distance) + '</span></div>';
                }
                var desc = (marker.marker_desc || '').replace(/(<([^>]+)>)/gi, "");
                var address = marker.address || '';
                
                var catBadges = '';
                if (marker.category_id && typeof wgm_categories_<?php echo esc_attr($count); ?> !== 'undefined') {
                    var cids = marker.category_id.toString().split(',');
                    cids.forEach(function(cid){
                        var c = wgm_categories_<?php echo esc_attr($count); ?>.find(function(item){ return item.id == cid; });
                        if(c) {
                            catBadges += '<span class="wgm-category-badge">' + escHtml(c.name) + '</span>';
                        }
                    });
                }
                if (catBadges) {
                    title += ' <div class="wgm-table-cat-wrap">' + catBadges + '</div>';
                }
            ...
            if ($.fn.DataTable.isDataTable('#wpgmap_marker_adv_table_<?php echo esc_attr($count); ?>')) {
            ...
            table = $('#wpgmap_marker_adv_table_<?php echo esc_attr($count); ?>').DataTable({
                ...
                createdRow: function (row, data, dataIndex) {
                    ...
                    // Add data-label for mobile card view
                    var labels = ["<?php esc_html_e('Icon', 'gmap-embed'); ?>", 
                                    "<?php esc_html_e('Image', 'gmap-embed'); ?>", 
                                    "<?php esc_html_e('Title', 'gmap-embed'); ?>", 
                                    "<?php esc_html_e('Description', 'gmap-embed'); ?>", 
                                    "<?php esc_html_e('Address', 'gmap-embed'); ?>", 
                                    "<?php esc_html_e('Actions', 'gmap-embed'); ?>"];
